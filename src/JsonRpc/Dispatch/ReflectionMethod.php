...
class ReflectionMethod implements MethodInterface
{
    ...
    public static function createFrom($classOrObject, $loadParameters = true, $aliasFactory = null)
    {
        ...
        // create an array of JsonRpc\Dispatch\ReflectionMethod
        $methods = array();
    ...
    public function invoke($arguments)
    {
        if (!is_callable($this->callable)) {
            throw new \JsonRpc\Exception_InternalError();
        }
        ...
        } catch (\JsonRpc\Exception $jex) {
            // allow user created exceptions that extend JsonRpc\Exception to bubble to client | error code
            throw $jex;
        } catch (\Exception $ex) {
    ...
    private function checkArguments($parameters, $arguments)
    {
        ...
        if (is_object($arguments)) { // associative list of parameter names and their values
            ...
            if (count($additionalArgs) > 0) {
                throw new \JsonRpc\Exception_InvalidParams();
            }
